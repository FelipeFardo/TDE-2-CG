...
int CScene5::DrawGLScene(void)	// Função que desenha a cena
{
	...
	if (GetTickCount() - contador >= 1000) {
		...
		DrawStacks(randomX, r, g, b);

		cubosAdicionadosNaPilha[randomX]++;

		cubosAdicionadosTotal++;

		bool todasPreenchidas = true;
		for (int i = 0; i < 5; i++) {
			if (cubosAdicionadosNaPilha[i] < 10) {
		...
		if (todasPreenchidas) {
			...
			for (int i = 0; i < 5; i++) {
				cubosAdicionadosNaPilha[i] = 0;
			}

			cubosAdicionadosTotal = 0;
		}
	}
...
void CScene5::DrawStacks(float x, float r, float g, float b) {


	for (int newY = 0; newY < 10; newY++) {
		if (posicaoCubo[(int)x][newY] == 0) {
